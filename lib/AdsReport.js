...
function AsyncDownload(rawRequest,options={***REMOVED***){
	...
	this.request = this.rawRequest = Object.assign({***REMOVED***,rawRequest);
	if(this.userInput){
		this.params = parseUserInputsToParams(rawRequest)
		this.request = Object.assign({***REMOVED***,this.params.params);
	***REMOVED***
...
AsyncDownload.prototype._startDownload = async function(reportId) {
	...
	const {token,debug=false,graphV***REMOVED*** = this.options;
	try{
...
AsyncDownload.prototype._start = async function(){
	try{
		...
		const {token,debug,graphV***REMOVED*** = this.options;
		let reportId = await createAsyncReport({
			accountId,token,
			params:Object.assign({***REMOVED***,request),
			graphV
		***REMOVED***);
		if(debug){
